...
const PdfViewer = ({ pdfUrl, initialPage = 1, height = "auto" }: PdfViewerProps) => {
    ...
    useEffect(() => {
        const handleResize = () => {
            setWidth(window.innerWidth > 768 ? window.innerWidth * 0.8 : window.innerWidth * 0.95);
        };
    ...
    return (
        <div className="pdf-container" style={{ height }}>
            ...
            >
                <button disabled={pageNumber <= 1} onClick={() => setPageNumber(prev => prev - 1)} style={{ padding: "8px 16px", fontSize: "16px" }}>
                    Previous
                </button>
